...
export default function IndustryInsightsFeed({ className, items: _rawItems }) {
          ...
          {items.map(({ title, image, publish_date, url_slug }, k) => (
            <Col key={k} xs={12} md={4} className="mb-4">
              <BlogFeedCard
                ...
                title={title}
                img={image}
                href={getRoute("industryInsightPage", { url_slug })}
              />
            </Col>
          ))}
        </Row>
...
IndustryInsightsFeed.defaultProps = {
  className: "py-5",
};

IndustryInsightsFeed.propTypes = {
