...
export default function CareerAdviceArticleContent({
  className,
  title,
  body,
  embed_media,
            ...
            {children.length > 0 ? (
              <SidebarNavigation
                ...
                items={[
                  {
                    label: title,
                    href: getRoute("careerAdvice", { url_slugs }),
                    children: children.map(({ title, url_slug }) => ({
                      label: title,
                      href: getRoute("careerAdvice", { url_slugs: [...url_slugs, url_slug] }),
                    })),
                  },
                ]}
              />
              ...
              siblings.length > 0 && (
                <SidebarNavigation
                  ...
                  items={[
                    {
                      label: parent.title,
                      href: getRoute("careerAdvice", { url_slugs: [...url_slugs.filter((i) => i !== url_slug)] }),
                      children: [
                        { label: title, href: getRoute("careerAdvice", { url_slugs }) },
                        ...siblings.map((s) => ({
                          label: s.title,
